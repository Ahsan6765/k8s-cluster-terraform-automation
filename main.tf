...
module "k8s_master" {
  ...
  ssh_private_key_path = var.ssh_private_key_path # Added this variable
  tags                 = merge(var.tags, { Name = each.key, Role = "master" })
}

# Kubernetes Worker Module
# Instantiated for all VMs that are NOT 'kmaster'
module "k8s_worker" {
  ...
  # Workers need to know where the join command is (generated by master)
  # Since master generates it to a local file, we point to that.
  # Note: Master must be applied first or concurrently. Terraform handles dependencies 
  # via implicit file usage if we modeled it, but here it's a local file.
  # To ensure Master runs first, we can make worker depend on master module.
